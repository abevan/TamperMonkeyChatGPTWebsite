...
(function () {
  ...
  function setTheme(name) {
    ...
    if (control) { control.dataset.label = label; control.title = `Theme: ${label}`; control.setAttribute('aria-label', `Cycle neon theme (current: ${label})`); }
  ...
  function mount() {
    ...
    if (!document.getElementById(CONTROL)) {
      ...
      c.id=CONTROL; c.type='button'; c.textContent='\u2726'; c.dataset.label='';
      c.setAttribute('aria-label','Cycle neon theme');
      const mobile = () => window.matchMedia('(max-width:900px)').matches;
      const collapseLater = () => { window.clearTimeout(c._neonCollapseTimer); c._neonCollapseTimer = window.setTimeout(() => { if (mobile()) c.classList.add('neon-mobile-collapsed'); }, 2800); };
      if (mobile()) c.classList.add('neon-mobile-collapsed');
      c.addEventListener('click',()=>{
        if (mobile() && c.classList.contains('neon-mobile-collapsed')) { c.classList.remove('neon-mobile-collapsed'); collapseLater(); return; }
        const keys=Object.keys(presets); setTheme(keys[(keys.indexOf(active)+1)%keys.length]);
        if (mobile()) collapseLater();
      });
  ...
  window.addEventListener('resize', () => { syncHorizon(); syncThemeControl(); }, { passive:true });
})();
